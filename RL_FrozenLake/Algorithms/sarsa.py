...
def sarsa(env, max_episodes, eta, gamma, epsilon, seed=None):
    ...
    for i in range(max_episodes):
        ...
        while not done:
            ...
            next_a = (random_state.randint(env.n_actions) if random_state.rand() < epsilon[i]
                      else np.random.choice(np.flatnonzero(q[next_s] == q[next_s].max())))
            q[s, a] += eta[i] * (reward + gamma * q[next_s, next_a] - q[s, a])
            s, a = next_s, next_a
